        ...
        <div class="card-body">
            <div id="owl-one" class="owl-carousel owl-theme" style="z-index: 0">
                @foreach(App\Gallery::where('slider', 1)->latest()->get() as $key => $item)
                    <a class="text-center" href="{{ asset(Storage::url($item->gallery)) }}" data-caption="{{ $item->caption }}" data-fancybox="informasi">
                        <img class="mw-100 gambar" style="height: 350px; object-fit: cover;" src="{{ asset(Storage::url($item->gallery)) }}" alt="{{ $item->caption }}">
                    </a>
                @endforeach
            </div>
        </div>
    ...
    <div class="card shadow mb-3">
        ...
        <div class="card-body overflow-auto" style="height: 500px">
            <div class="list-group">
                @foreach (App\Surat::select('id','nama')->whereTampilkan(1)->orderBy('nama')->get() as $item)
                ...
                @foreach(App\PemerintahanDesa::select('foto','nama','jabatan')->orderBy('urutan')->get() as $key => $item)
                    <div class="text-center">
                        <a class="text-center" href="{{ $item->foto ? asset(Storage::url($item->foto)) : asset('storage/noavatar.png') }}" data-caption="{{ $item->nama }} {{ $item->jabatan ? '(' . $item->jabatan . ')' : '' }}" data-fancybox>
                            <img class="mw-100 gambar" src="{{ $item->foto ? asset(Storage::url($item->foto)) : asset('storage/noavatar.png') }}" alt="Pemerintahan Desa {{ $key }}">
                        </a>
